...
"""
Wrtitten Marts 2018 -- Nicholas Jannsen
Typeset in Python 3
...
argm = 6             # Max number of arguments parsed
...
def help():
    ...
    Usage : %s <method> <color> <C> <[M/Fe]> <star>

    method: For now either                                [string]
    star  : Dwarf ('dwarf') or Giant ('giant')            [string]
    color : The color used e.g. 'B-V'                     [string]
    C     : The true color; color corrected for reddening [int]
    [M/Fe]: Metallicity                                   [float]
...
def T(method, color='B-V', C=1.0, M=0.0, star='dwarf'):
    ...
    if method=='alonso' or method=='torres' or method=='sausa':
        try: a
            ...
            print('ERROR: COLOR OR METALLICITY IS NOT INSIDE APPLICABLE RANGE!'); return
    if method=='remirez':
        try: P
        except NameError:
            print('ERROR: COLOR OR METALLICITY IS NOT INSIDE APPLICABLE RANGE!'); return
